...
export default function EllaS({ data }) {
  const [timePeriod, setTimePeriod] = useState('30d');
  
  // Find Ella S's data in the array
  const consultant = findConsultant(data, 'Ella S');

  if (!consultant) return <div>No data available for Ella S</div>;
